 ...
 * 更新日期：2026-04-12 支持从 web 页面获取 cache 信息
...
async function translate_logs() {
  ...
  try {
    ...
    if (translatedArr.length === logs.length) {  // 判断数量是否一致
      ...
      $.log(`翻译: ${logs.length} 次, 用时 ${costTime} 秒 🎉`);
      return;
    }
  } catch (e) {
    $.log(`❌ 翻译异常: ${e}`);
...
async function translate_cache() {
  ...
  // 拼接所有要翻译的内容
  const combinedText = [name, hints, longDescription].join("\n\n=====\n\n");
  try {
    // 调用翻译接口
    const translatedCombined = await translateApi(combinedText);
    // 符切分回 3 段译文
    ...
    if (translatedArr.length >= 3) {
      ...
      if (_longDescription != longDescription) body.longDescription = _longDescription + `\n--------------------------------------------------\r\n ` + longDescription;
      $.log("✅ cache 翻译完成");
    }
  } catch (e) {
    $.log(`❌ cache 翻译异常: ${e}`);
