 ...
 * 更新日期：2026-04-11 解锁 D2/T2.5 缓存APP访问权限
...
!(async () => {
  ...
  if (/map\/search\?adventuresTake/.test($request.url)) {
    ...
    body = {
      ...
      "longDescription": "Happy Geocaching!",
    ...
    var openUrl = 'https://www.geocaching.com/geocache/' + gc_code;
    $.msg(name, ``, openUrl, { $open: openUrl });

  } else if (/geocaches\/GC[A-Z0-9]{5}\/(userwaypoints|additionalwaypoints)/.test($request.url)) {
    if (geocaching_gps_fix == 'false') throw new Error('⚠️ 未启用转换坐标功能');
    // 转换 userwaypoints/additionalwaypoints 坐标
    if (body?.data[0]?.coordinates) {
      body.data[0].coordinates = convertCoordinates(body.data[0].coordinates);
      $.log("✔️ 坐标转换完成");
    }
  } else {
...
async function translate_cache() {
  $.log("🌏 开始翻译 cache");
  let { name, hints, longDescription, difficulty, terrain, referenceCode } = body;  // 标题, 提示, 描述, 难度, 地形, 编码
